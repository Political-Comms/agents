...
- **Use suppressions to stop mailing an address.** `POST /v1/email/suppressions` is the public primitive; it survives a re-import, because the underlying contact rows carry the bounce and complaint history. There is no public contact-delete. Scope is `org`, `identity`, or `domain`, and all three apply automatically - there is nothing to attach to a campaign. To hold a set of people out of ONE campaign instead, put an ordinary email list in the campaign's `suppression_list_ids`: any list can be an audience or a hold-out, and the campaign decides which.
...
- **Importing a list is one call over a URL you host, and the file IS the list.** `POST /v1/email/lists/import` fetches an HTTPS CSV (50 MB cap, SSRF-guarded), stages it, and commits it as a NEW list, answering `202`; progress shows on the list in the dashboard. There is no target list to choose: `name` defaults to the file name, and `email_domain_id` scopes the new list to one sending domain (omit it for a list any campaign may use). A campaign combines lists by attaching several. `mapping` is optional and common ESP exports are recognized; when no email column is found the call returns `400 VALIDATION_ERROR` with `details.headers` listing what was read, so send a mapping naming the right column rather than retrying the same body.
