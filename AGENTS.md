...
- **A validation export is polled, and `409` is the signal.** `POST /v1/email/lists/{id}/export` answers `202` with a `file_id`; `GET /v1/email/lists/{id}/export/{fileId}/download` answers `409 EXPORT_NOT_READY` until the worker has built the file, so treat that as "poll again" rather than an error, and do not re-queue. Send `{"state": "undeliverable"}` to narrow the CSV to one verdict class, or one of three presets: `max_deliverability` (deliverable only), `max_reach` (deliverable, risky, and unknown, minus disposable and role mailboxes), or `only_bad` (undeliverable plus disposable and role mailboxes). The presets exclude addresses that have never been validated; `all` is the only filter that includes them. Blank verdict columns mean the address has no cached verdict (never validated, or the 90-day cache expired), which is not the same as `false`.
- **`recipient_policy` decides who actually gets a campaign.** Email campaigns (`GET/POST/PATCH /v1/email/campaigns...`) carry `recipient_policy`: `max_reach` (default) sends to every subscribed contact; `max_deliverability` sends only to contacts whose current validation verdict is deliverable, skipping contacts that have never been validated. Writable on create and update, readable on every campaign response.
